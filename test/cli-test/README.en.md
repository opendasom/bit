...
- A fresh contract is deployed on every run, so repeatedly running this
  ...
  run's repoId.
- `.bit/config.json` stores the private key in plaintext, but it only
  ever exists inside `$WORKDIR` (a temp directory) and is deleted when
  the script exits (unless `KEEP_WORKDIR=1`).
